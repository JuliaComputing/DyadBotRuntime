...
function compute_pwm!(ctrl::BalanceController{ParallelPID},
    ...
    ctrl.encoder_left_pulse += encoder_count_left
    ctrl.encoder_right_pulse += encoder_count_right

    # Get calibrated angles from Kalman filter
    accel_angle, gyro_x, gyro_z = compute_angles(ax, ay, az, gx, gy, gz)

    # Update Kalman filter
    innovation = update!(ctrl.kf, gyro_x, accel_angle)
    ...
    return kalman_angle, innovation
end
