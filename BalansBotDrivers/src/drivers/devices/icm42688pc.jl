...
function set_lpf_config!(imu::ICM42688PC, accel_lpf_mode::Union{Nothing, LpfMode}, gyro_lpf_mode::Union{Nothing, LpfMode})
    has_accel = !isnothing(accel_lpf_mode)
    has_gyro = !isnothing(gyro_lpf_mode)
    ctrl5 = CTRL5(has_accel, has_accel ? UInt8(accel_lpf_mode) : 0, has_gyro, has_gyro ? UInt8(gyro_lpf_mode) : 0)
    write_reg(imu, CTRL5_ADDR, as_byte(ctrl5))
    return nothing
end
